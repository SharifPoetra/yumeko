...
module.exports.run = async (client, msg, args) => {
  ...
  if (!msg.member.voice.channel) return msg.channel.send("Join voice channel first");
  if (!serverQueue) return msg.channel.send("No songs to seek");
  if (serverQueue.voiceChannel.id !== msg.member.voice.channel.id) return msg.chanel.send(`You must in **${serverQueue.voiceChannel.name}** to seek the song`);
  try {
    ...
    serverQueue.endReason = `seek ${optDurr}`;
